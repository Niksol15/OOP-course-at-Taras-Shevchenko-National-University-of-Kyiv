...
//Тут такой порушено принцип відкритості/закритості

abstract class EmailLogger
{
    public abstract void Log(Email email);
}

class ConsoleEmilLogger: EmailLogger
{
    public override void Log(Email email)
    {
...
class Program
{
    static void Main(string[] args)
    {
        ...
        es.Logger = new ConsoleEmilLogger();
        es.Send(e1);
        es.Send(e2);
        es.Send(e3);
        es.Send(e4);
